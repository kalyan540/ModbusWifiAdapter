...
ModbusWifiAdapter::ModbusWifiAdapter(Stream& serial, int8_t dePin, int8_t rePin) : _rtuComm(serial, dePin, rePin) {
  _rtuComm.setTimeout(500);
}

void ModbusWifiAdapter::setTimeout(unsigned long timeout) {
  _rtuComm.setTimeout(timeout);
}

void ModbusWifiAdapter::begin(unsigned long baud, uint32_t config) {
  _rtuComm.begin(baud, config);
}



ModbusWifiAdapterError ModbusWifiAdapter::readCoils(uint8_t id, uint16_t startAddress, bool buf[], uint16_t quantity) {
  return _readValues(id, 1, startAddress, buf, quantity);
}

ModbusWifiAdapterError ModbusWifiAdapter::readDiscreteInputs(uint8_t id, uint16_t startAddress, bool buf[], uint16_t quantity) {
  return _readValues(id, 2, startAddress, buf, quantity);
}

ModbusWifiAdapterError ModbusWifiAdapter::readHoldingRegisters(uint8_t id, uint16_t startAddress, uint16_t buf[], uint16_t quantity) {
  return _readValues(id, 3, startAddress, buf, quantity);
}

ModbusWifiAdapterError ModbusWifiAdapter::readInputRegisters(uint8_t id, uint16_t startAddress, uint16_t buf[], uint16_t quantity) {
  return _readValues(id, 4, startAddress, buf, quantity);
}



ModbusWifiAdapterError ModbusWifiAdapter::writeSingleCoil(uint8_t id, uint16_t address, bool value) {
  return _writeSingleValue(id, 5, address, ((value) ? 0xFF00 : 0x0000));
}

ModbusWifiAdapterError ModbusWifiAdapter::writeSingleHoldingRegister(uint8_t id, uint16_t address, uint16_t value) {
  return _writeSingleValue(id, 6, address, value);
}



ModbusWifiAdapterError ModbusWifiAdapter::writeMultipleCoils(uint8_t id, uint16_t startAddress, bool buf[], uint16_t quantity) {
...
ModbusWifiAdapterError ModbusWifiAdapter::writeMultipleHoldingRegisters(uint8_t id, uint16_t startAddress, uint16_t buf[], uint16_t quantity) {
...
uint8_t ModbusWifiAdapter::getExceptionResponse() {
  return _exceptionResponse;
}



ModbusWifiAdapterError ModbusWifiAdapter::_readValues(uint8_t id, uint8_t functionCode, uint16_t startAddress, bool buf[], uint16_t quantity) {
...
ModbusWifiAdapterError ModbusWifiAdapter::_readValues(uint8_t id, uint8_t functionCode, uint16_t startAddress, uint16_t buf[], uint16_t quantity) {
  if (id < 1 || id > 247) return MODBUS_RTU_MASTER_INVALID_ID;
  if (!buf) return MODBUS_RTU_MASTER_INVALID_BUFFER;
...
ModbusWifiAdapterError ModbusWifiAdapter::_writeSingleValue(uint8_t id, uint8_t functionCode, uint16_t address, uint16_t value) {
  if (id > 247) return MODBUS_RTU_MASTER_INVALID_ID;
  ModbusADU adu;
  adu.setUnitId(id);
  adu.setFunctionCode(functionCode);
...
ModbusWifiAdapterError ModbusWifiAdapter::_translateCommError(ModbusRTUCommError commError) {
  switch (commError) {
